...
def train(args):
    ...
    transform = transforms.Compose([
        ...
        transforms.Normalize(
            [0.5 for _ in range(model_config['im_channels'])],[0.5 for _ in range(model_config['im_channels'])]
        )
    ])
    
    dataset = datasets.ImageFolder(root=dataset_config["im_path"],
                               transform=transform)
    ...
    for epoch_idx in range(num_epochs):
        for images, _ in tqdm(dataloader):  # ignore labels with "_"
