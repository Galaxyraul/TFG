...
# === Config ===
model_path = '/raid/rgt00024/tfg/from_scratch/gans/wgan_gp/checkpoints/drones/100/gen.pt'        # Your pretrained model (.pth)
...
# === Load model ===
from wgan_gp import Generator    # Replace with your actual model class
model = Generator(128,3,64).to(device)
...
for i in tqdm(range(num_images), desc="Generating"):
    ...
    # Forward pass
    with torch.no_grad():
        img = model(z, class_labels) if class_labels is not None else model(z)
